# K8SJKS - Java Keystores in Kubernetes Secrets

Manages a Keyfactor Command certificate store for Java Keystores (JKS) stored as base64-encoded data in Kubernetes Opaque secrets.

JKS keystores require a password, which can be provided directly or referenced from a separate Kubernetes secret ("buddy password" pattern).

## Usage

### Basic JKS store with direct password
...
### JKS store with buddy password (separate K8S secret)
...
JKS keystores require a password. You have two options:
...
| certificate_data_field_name | Field name for JKS data in the K8S secret. | `string` | `null` | no |
...
| store_type | The store type (always K8SJKS). |
