...
public class K8STLSSecrStoreIntegrationTests : IntegrationTestBase
{
    ...
    public async Task Management_AddCertificateWithChain_IncludeCertChainFalse_OnlyLeafCertStored()
    {
        ...
        var secretName = $"test-no-chain-tls-{Guid.NewGuid():N}";
        TrackSecret(secretName);
        ...
        var jobConfig = new ManagementJobConfiguration
        {
            ...
            CertificateStoreDetails = new CertificateStore
            {
                ClientMachine = TestNamespace,
                StorePath = secretName,
                Properties = $"{{\"KubeSecretType\":\"tls_secret\",\"KubeNamespace\":\"{TestNamespace}\",\"IncludeCertChain\":false}}"
            },
            ServerUsername = string.Empty,
            ServerPassword = KubeconfigJson,
            UseSSL = true,
    ...
    [SkipUnless(EnvironmentVariable = "RUN_INTEGRATION_TESTS")]
    public async Task Management_CreateStoreIfMissing_NoCertificateData_CreatesEmptyTlsSecret()
    {
        ...
        // Verify secret was created as TLS secret type
        var secret = await K8sClient.CoreV1.ReadNamespacedSecretAsync(secretName, TestNamespace);
        Assert.NotNull(secret);
        Assert.Equal("kubernetes.io/tls", secret.Type);
    }
    ...
    public async Task Inventory_TlsSecretWithMultipleCertsInCaCrt_ReturnsAllCertificates()
    {
        ...
        // Cached chain has: leaf CN = "Multi CA Inventory Test", intermediate = "Intermediate CA (Rsa2048)", root = "Root CA (Rsa2048)"
        Assert.Contains(certSubjects, s => s.Contains("Multi CA Inventory Test"));
        Assert.Contains(certSubjects, s => s.Contains("Intermediate"));
    ...
    {
        // Arrange - Create a TLS secret directly in the "default" namespace
        var secretName = $"test-tls-default-ns-{Guid.NewGuid():N}";
        var certInfo = CachedCertificateProvider.GetOrCreate(KeyType.Rsa2048, "TLS Default Namespace Test Cert");
        var certPem = CertificateTestHelper.ConvertCertificateToPem(certInfo.Certificate);
        var keyPem = CertificateTestHelper.ConvertPrivateKeyToPem(certInfo.KeyPair.Private);
        ...
        try
        {
            ...
            var jobConfig = new InventoryJobConfiguration
            {
                ...
                CertificateStoreDetails = new CertificateStore
                {
                    ...
                    Properties = "{\"KubeSecretType\":\"tls_secret\"}"  // No KubeNamespace specified
                },
                ServerUsername = string.Empty,
                ServerPassword = KubeconfigJson,
                UseSSL = true
    ...
    {
        // Arrange
        var secretName = $"test-tls-inferred-ns-{Guid.NewGuid():N}";
        await CreateTestTlsSecret(secretName, KeyType.Rsa2048);
        ...
        var jobConfig = new InventoryJobConfiguration
        {
            ...
            CertificateStoreDetails = new CertificateStore
            {
                ...
                Properties = "{\"KubeSecretType\":\"tls_secret\"}"  // No KubeNamespace - should infer from path
            },
            ServerUsername = string.Empty,
            ServerPassword = KubeconfigJson,
            UseSSL = true
